...
1. We are targeting Microsoft Semantic Kernel (`Microsoft.SemanticKernel` and related packages - `Agents.Core`, `Agents.Orchestration`, `Agents.Runtime.InProcess`, `Connectors.Google`, etc.) on .NET 10. Before writing code for any given day, verify current package names, namespaces, and API signatures - anything under the `Agents.*` or `Process.*` namespaces remains experimental/preview and can shift between releases, or simply not work as documented yet. Flag any breaking changes you find versus what's shown in older samples. If a lesson's experimental SK package proves unreliable in practice (reproduce it live before concluding this - see Day 34), say so explicitly and hand-roll the pattern with plain `Kernel`/`IChatCompletionService` calls instead of shipping a flaky lesson or reaching for a different framework.
2. We are EXCLUSIVELY using Google Gemini as the model provider, via Semantic Kernel's native Google connector (`Microsoft.SemanticKernel.Connectors.Google`, `AddGoogleAIGeminiChatCompletion`). If a given lesson's SK feature has no working integration with the Gemini connector at time of writing, identify the current best-supported alternative and note that fact explicitly in the lesson's Setup section so viewers aren't confused.
3. The chat model is always gemini-2.5-flash. The embedding model (where relevant, e.g. memory/RAG days) is always gemini-embedding-001 (text-embedding-004 was retired by Google - confirmed 404/not-in-model-list as of Day 5 research on 2026-08-04 - and gemini-embedding-001 is the current stable replacement).
4. API keys must always be retrieved from environment variables (e.g., GEMINI_API_KEY). Never hardcode keys.
